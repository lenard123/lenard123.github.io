<template>
  ...
  <div class="max-w-3xl px-4 mx-auto sm:px-6 xl:max-w-5xl xl:px-0 pt-20">
    ...
    <div class="pt-6 pb-8 space-y-2 md:space-y-5">
      ...
      <h1 class="text-3xl font-bold leading-9 tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl sm:leading-10 md:text-6xl md:leading-14">{{ tag }}</h1>
...
export default {
  ...
  setup() {
    ...
    const posts = computed(() => store.state.posts.filter(post => post.tags.includes(tag.value)))

    onMounted(() =>{
      scrollToTop()
      setPageTitle(tag.value)
    })
